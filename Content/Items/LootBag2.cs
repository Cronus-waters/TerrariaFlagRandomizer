...
{
	internal class LootBag2 : ModItem
	{
		...
		public override void RightClick(Player player)
		{
			...
			// Progressive bar
			int type = Main.rand.Next(TieredItemSets.PostSkeletronBars);
			...
			{
				int index = Main.rand.Next(TieredItemSets.PostSkeletronMaterials.GetLength(0));
				type = TieredItemSets.PostSkeletronMaterials[index, 0];
				int min = TieredItemSets.PostSkeletronMaterials[index, 1];
				int max = TieredItemSets.PostSkeletronMaterials[index, 2] + 1;
			...
			for (int i = 0; i < times; i++)
			{
				type = Main.rand.Next(TieredItemSets.PostSkeletronWeapons);
