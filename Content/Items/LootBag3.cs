...
namespace TerrariaFlagRandomizer.Content.Items
{
	internal class LootBag3 : ModItem
	{
		...
		public override void RightClick(Player player)
		{
			...
			// Progressive bar
			int type = Main.rand.Next(TieredItemSets.HardmodeBars);
			...
			{
				int index = Main.rand.Next(TieredItemSets.HardmodeMaterials.GetLength(0));
				type = TieredItemSets.HardmodeMaterials[index, 0];
				int min = TieredItemSets.HardmodeMaterials[index, 1];
				int max = TieredItemSets.HardmodeMaterials[index, 2] + 1;
			...
			for (int i = 0; i < times; i++)
			{
				type = Main.rand.Next(TieredItemSets.HardmodeWeapons);
