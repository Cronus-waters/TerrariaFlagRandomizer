...
{
	internal class LootBag4 : ModItem
	{
		...
		public override void RightClick(Player player)
		{
			...
			// Progressive bar
			int type = Main.rand.Next(TieredItemSets.PostMechBars);
			...
			{
				int index = Main.rand.Next(TieredItemSets.PostMechMaterials.GetLength(0));
				type = TieredItemSets.PostMechMaterials[index, 0];
				int min = TieredItemSets.PostMechMaterials[index, 1];
				int max = TieredItemSets.PostMechMaterials[index, 2] + 1;
			...
			for (int i = 0; i < times; i++)
			{
				type = Main.rand.Next(TieredItemSets.PostMechWeapons);
			...
			{
				type = Main.rand.Next(TieredItemSets.Wings);
				player.QuickSpawnItem(source, type);
			}
