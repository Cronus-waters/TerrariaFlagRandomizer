...
namespace TerrariaFlagRandomizer.Content.Items
{
	internal class LootBag6 : ModItem
	{
		...
		public override void RightClick(Player player)
		{
			...
			// Progressive bar
			int type = Main.rand.Next(TieredItemSets.PostGolemBars);
			...
			{
				int index = Main.rand.Next(TieredItemSets.PostGolemMaterials.GetLength(0));
				type = TieredItemSets.PostGolemMaterials[index, 0];
				int min = TieredItemSets.PostGolemMaterials[index, 1];
				int max = TieredItemSets.PostGolemMaterials[index, 2] + 1;
			...
			for (int i = 0; i < times; i++)
			{
				type = Main.rand.Next(TieredItemSets.PostGolemWeapons);
